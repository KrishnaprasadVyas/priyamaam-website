...
function About({ content, profile }) {
  const expertise = ['AI', 'Machine Learning', 'Computer Vision', 'Deep Learning']
  const roles = [
    {
      title: 'Assistant Professor',
      detail: 'Teaching undergraduate engineering courses with focus on AI-driven applications.',
    },
    {
      title: 'Research Coordinator',
      detail: 'Leading publication, project, and innovation initiatives within the department.',
    },
    {
      title: 'Mentor',
      detail: 'Guiding student teams for research projects, competitions, and career development.',
    },
  ]
      ...
      >
        ...
        >
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-gray-500 sm:text-base">{content}</p>
          </div>
          ...
          >
            <h2 className="text-base font-semibold text-gray-900">Profile Card</h2>
            <dl className="mt-4 space-y-3 text-sm">
              ...
              <div>
                ...
                <dd className="font-medium text-gray-900">{profile.subtitle}</dd>
              </div>
              <div>
                <dt className="text-gray-500">Department</dt>
                <dd className="font-medium text-gray-900">Computer Engineering</dd>
              </div>
              <div>
                <dt className="text-gray-500">College</dt>
                <dd className="font-medium text-gray-900">{profile.organization}</dd>
              </div>
            </dl>
          </motion.div>
        </motion.div>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-gray-900">Experience & Roles</h2>
          <motion.div
          ...
          >
            {roles.map((role) => (
              ...
              >
                ...
                <p className="mt-2 text-sm leading-relaxed text-gray-500">{role.detail}</p>
              </motion.article>
            ))}
          </motion.div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <motion.div
            ...
            variants={fadeUp}
          >
            <h2 className="text-xl font-semibold text-gray-900">Career Timeline</h2>
            <motion.div className="mt-4 space-y-3" variants={staggerContainer} initial="hidden" animate="visible">
              {timeline.map((item) => (
                <motion.div key={item.year} className="flex items-center gap-3" variants={staggerItem}>
